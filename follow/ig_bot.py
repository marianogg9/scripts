...
def getOptions(args=sys.argv[1:]):
    ...
    parser.add_argument("-d", "--path", type=str, help="Web driver full path.", default="C:\\Users\default\chromedriver_win32\chromedriver.exe")
    parser.add_argument("-a", "--hashtag", type=str, help="Which hashtag(s) to query. More than one? just concatenate them as a list of words.",
                        required=True)
    parser.add_argument("-n", "--num", type=int, help="How many photos to loop through.", required=True)
    parser.add_argument("-u", "--username", type=str, help="Instagram username.", required=True)
    parser.add_argument("-p", "--password", type=str, help="Instagram password.", required=True)
    options = parser.parse_args(args)
    return options

def get_driver():
    chromedriver_path = 'C:\\Users\Mariano_Gonzalez\Downloads\chromedriver_win32\chromedriver.exe'  # Change this to your own chromedriver path!
